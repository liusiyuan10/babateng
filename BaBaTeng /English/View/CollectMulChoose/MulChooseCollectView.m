...
//    _choosedArr = [NSMutableArray arrayWithObjects:@"1",@"4",@"5",nil];
    ...
    _MyCollectView.backgroundColor = [UIColor colorWithRed:247/255.0 green:247/255.0 blue:247/255.0 alpha:1.0];
...
//    cell.titleLab.text = [_dataArr objectAtIndex:indexPath.row];
    ...
    TeaReservationTime *reservationtime = [_dataArr objectAtIndex:indexPath.row];
    
    cell.titleLab.text = reservationtime.connectSeTime;
//    cell.SelectIconBtn.titleLabel.text = reservationtime.connectSeTime;
//    [cell.SelectIconBtn setTitle:reservationtime.connectSeTime forState:UIControlStateNormal];
    
    NSLog(@"cell.titleLab.text====%@",cell.titleLab.text);
    
    for (int i = 0; i < _dataArr.count; i++) {
      NSLog(@"all no cell.titleLab.text====%@",cell.titleLab.text);
    [cell UpdateCellWithState:NO];
        
    }

    for (int i = 0; i < _choosedArr.count; i++) {
//        NSString *str = [_choosedArr objectAtIndex:i];
        TeaReservationTime *reservationchosetime = [_choosedArr objectAtIndex:i];
//        NSLog(@"str======%@",str);
        if ([reservationtime.courseId isEqualToString:reservationchosetime.courseId]) {

//            NSLog(@"strsssssssss======%@",str);
...
//        [_choosedArr addObject:cell.titleLab.text];
        ...
        [_choosedArr addObject:reservationtime];
    }
    else{
//        [_choosedArr removeObject:cell.titleLab.text];
        
         [_choosedArr removeObject:reservationtime];
    }
...
//    _block(cell.titleLab.text,_choosedArr);
    ...
    _block(reservationtime.courseId,_choosedArr);
}
