...
//    UIButton *replayBtn = [[UIButton alloc] initWithFrame:CGRectMake(50, 50, 100, 100)];
//
...
//    self.replayBtn.frame = self.bounds;
...
//    }
        ...
        if (self.isLandscape) { // 转至竖屏
            ...
            if ([self.delegate respondsToSelector:@selector(YGPlayerViewrotateBtnClicked:RType:)]) {
                
                [self.delegate YGPlayerViewrotateBtnClicked:self RType:@"0"];
            }
        } else { // 转至横屏
            NSLog(@"转至横屏");
            ...
            if ([self.delegate respondsToSelector:@selector(YGPlayerViewrotateBtnClicked:RType:)]) {
                ...
                [self.delegate YGPlayerViewrotateBtnClicked:self RType:@"1"];
            }
        }
...
//    [UIDevice switchNewOrientation:deviceOrientation];
...
- (void)showOrHideControlPanel
{
    if (self.controlPanelIsShowing) {
        ...
        [UIView animateWithDuration:.5f animations:^{
            ...
            if (self.isLandscape) {
                [self performSelector:@selector(hideStatusBar) withObject:nil afterDelay:2.f];
            }
        }];
    }
}
...
- (void)autoFadeOutControlPanel {
    [NSObject cancelPreviousPerformRequestsWithTarget:self];
    [self performSelector:@selector(hideControlPanel) withObject:nil afterDelay:2.f];
}
...
- (void)YGbackForePage
{
    NSLog(@"YGbackForePagesdfdf -----d-fd-f-d-f-d-f-d-f-d-f");
...
- (IBAction)back
{
    NSLog(@"sdfdf -----d-fd-f-d-f-d-f-d-f-d-f");
...
//    [self destroyPlayer];
    ...
    if ([self.delegate respondsToSelector:@selector(YGPlayerViewBtnClicked:)]) {
        
        [self.delegate YGPlayerViewBtnClicked:self];
    }
...
- (void)dealloc
{
      NSLog(@"dealloc -----d-fd-f-d-f-d-f-d-f-d-f");
