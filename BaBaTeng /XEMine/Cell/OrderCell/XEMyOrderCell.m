...
-(UIView*)contentViewCell{
    ...
    _combinedLabel = [[UILabel alloc] initWithFrame:CGRectMake( CGRectGetMaxX(_totalLabel.frame) + 8, CGRectGetMaxY(_leftImage.frame) + 21 ,45, 11)];
    
    _combinedLabel.font = [UIFont systemFontOfSize:12.0];
    _combinedLabel.backgroundColor = [UIColor clearColor];
    _combinedLabel.textColor = [UIColor colorWithRed:144/255.0 green:144/255.0 blue:144/255.0 alpha:1.0];
    _combinedLabel.text = @"需付款:";
    _combinedLabel.textAlignment = NSTextAlignmentRight;
    
    [backView addSubview:_combinedLabel];
    
    _priceLabel = [[UILabel alloc] initWithFrame:CGRectMake( CGRectGetMaxX(_combinedLabel.frame), CGRectGetMaxY(_leftImage.frame) + 18 ,130, 15)];
...
//    _payBtn.layer.borderColor = MNavBackgroundColor.CGColor;
    ...
    _payagBtn = [[UIButton alloc]initWithFrame:CGRectMake(16,CGRectGetMaxY(lineView.frame) + 6 ,backViewW - 32, 33)];
    _payagBtn.backgroundColor = [UIColor clearColor];
    _payagBtn.contentMode = UIViewContentModeScaleAspectFill;
    
    [_payagBtn setTitleColor:MNavBackgroundColor forState:UIControlStateNormal];
    [_payagBtn setTitle:@"再次购买" forState:UIControlStateNormal];
    
    //    [_phoneBtn addTarget:self action:@selector(ExperienceBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    
    _payagBtn.titleLabel.font = [UIFont systemFontOfSize:12.0];
    
    _payagBtn.layer.borderWidth = 1.0f;
    
    _payagBtn.layer.borderColor = MNavBackgroundColor.CGColor;
    
    _payagBtn.layer.cornerRadius= 5.0f;
    
    _payagBtn.clipsToBounds = YES;//去除边界
    
    [backView addSubview:_payagBtn];
