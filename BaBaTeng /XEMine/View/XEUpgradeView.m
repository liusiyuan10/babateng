...
- (void)initContent
{
    ...
    self.LevelLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(headView.frame) + 23, kDeviceWidth , 24)];
    
    
    self.LevelLabel.textColor = [UIColor colorWithRed:54/255.0 green:54/255.0 blue:54/255.0 alpha:1.0];
    self.LevelLabel.font = [UIFont systemFontOfSize:18.0];
    self.LevelLabel.textAlignment = NSTextAlignmentCenter;
    
    

    
    self.priceLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.LevelLabel.frame) + 23, kDeviceWidth , 24)];
    ...
    if (_contentView == nil)
    {
        ...
        [_contentView addSubview:self.LevelLabel];
...
-(void)determineBtnClicked:(UIButton *)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(XEUpgradeViewBtnClicked:)]) {

        [self.delegate XEUpgradeViewBtnClicked:self];
    }
    [self disMissView];
}
...
- (void)showInView:(UIView *)view
{
    ...
    self.priceLabel.text = self.priceStr;
    self.LevelLabel.text = [NSString stringWithFormat:@"升级会员:%@",self.levelStr];
