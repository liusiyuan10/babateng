...
@interface XEMyOrderViewController ()<UITableViewDelegate,UITableViewDataSource,QAlbumPayViewDelegate>
...
@implementation XEMyOrderViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(XEMyOrderViewRefresh:) name:@"XEMyOrderViewRefresh" object:nil];
...
- (void)getOrder
{
    ...
    [MineRequestTool GetAllmallOrderpageNum:[NSString stringWithFormat:@"%ld",(long)self.PageNum] success:^(XEOrderModel * _Nonnull response) {
        [self stopLoading];
...
- (void)XEMyOrderViewRefresh:(NSNotification *)noti
{
    self.PageNum = 1;
    [self getOrder];
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    static NSString *cellIndentifierOne = @"XEMyOrderCellcell";
    XEMyOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifierOne];
    
    if (!cell) {
        
        cell = [[XEMyOrderCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifierOne];
    ...
    if ([listdata.orderStatus isEqualToString:@"0"]) {
        cell.orderStaueLabel.text = @"等待买家付款";
        cell.combinedLabel.text = @"需付款:";
    }else if ([listdata.orderStatus isEqualToString:@"1"] || [listdata.orderStatus isEqualToString:@"2"])
    {
        cell.orderStaueLabel.text = @"等待买家收货";
        cell.combinedLabel.text = @"实付款:";
    }else if ([listdata.orderStatus isEqualToString:@"3"])
    {
    ...
    if ([listdata.orderStatus isEqualToString:@"0"]) {
        ...
        cell.payagBtn.hidden = YES;
    }else
    {
        cell.payBtn.hidden = YES;
        cell.payagBtn.hidden = NO;
    }
    ...
    cell.payagBtn.tag = indexPath.row;
    [cell.payagBtn addTarget:self action:@selector(payagBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if ([listdata.orderStatus isEqualToString:@"1"])
    {
...
- (void)payagBtnClicked:(UIButton *)btn
{
...
//    NSString *priceStr = [NSString stringWithFormat:@"¥%.2f",listdata.goodsTotalPrice];
