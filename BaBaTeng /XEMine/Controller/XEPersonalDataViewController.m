...
@interface XEPersonalDataViewController ()<UITableViewDelegate,UITableViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate,VPImageCropperDelegate,CityListViewDelegate,NickNameViewDelegate, UIAlertViewDelegate,CustomSheetViewDelegate>{
...
@implementation XEPersonalDataViewController
...
-(void)logOutAction{
    ...
    __block XEPersonalDataViewController *self_c = self;
    //点击按钮回调方法
            ...
            switch (indexPath.row) {
                case 0:
                {
                    ...
                    if (self.touxiangImage !=nil) {
                        ...
                        [[XEMineViewController getInstance] updateImage: self.touxiangImage];
                        //
                    }else{
                        
                        NSLog(@"userIcon====%@",[[TMCache sharedCache] objectForKey:@"userIcon"]);
                        
                        [cell.rightImage setImageWithURL:[NSURL URLWithString: [[TMCache sharedCache] objectForKey:@"userIcon"]] placeholderImage:[UIImage imageNamed:@"user"]];
