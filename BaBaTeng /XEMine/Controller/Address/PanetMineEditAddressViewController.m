//
//  PanetMineEditAddressViewController.m
//  BaBaTeng
//
...
#define PanetMineEditAddress_MAX_LIMIT_NUMS     50

@interface PanetMineEditAddressViewController ()<UITextFieldDelegate,UITextViewDelegate>
{
...
@implementation PanetMineEditAddressViewController
...
- (void)viewDidLoad {
    ...
    self.provinceStr = self.listdata.province;
    self.cityStr = self.listdata.city;
    self.townStr = self.listdata.area;
...
- (void)LoadChlidView
{
    ...
    self.DeleteBtn = [[UIButton alloc] initWithFrame:CGRectMake( 50, KDeviceHeight - 64 - 44 - 20, kDeviceWidth - 100, 44)];
    
    [self.DeleteBtn setTitle:@"删除联系人" forState:UIControlStateNormal];
    ...
    self.DeleteBtn.titleLabel.font = [UIFont systemFontOfSize:14.0f];
    
    self.DeleteBtn.backgroundColor = [UIColor redColor];
    self.DeleteBtn.layer.cornerRadius=22.0f;
    self.DeleteBtn.layer.masksToBounds = YES; //没这句话它圆不起来
    
    [self.view addSubview:self.DeleteBtn];
    
    
//    self.DeleteBtn.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1.0];
//    self.DeleteBtn.font = [UIFont systemFontOfSize:14.0];
//    self.DeleteBtn.text = @"删除联";
...
- (void)LoadinformationView
{
    ...
    self.nametext.text = self.listdata.receiverName;
    ...
    self.phonetext.text = self.listdata.receiverPhone;
    ...
    self.areatextlabel.text = [NSString stringWithFormat:@"%@ %@ %@", self.listdata.province,self.listdata.city,self.listdata.area];
    ...
    self.addrestextView.text = self.listdata.address;
...
- (void)SaveAddress
{
    ...
    NSDictionary *parameter = @{@"address" : self.addrestextView.text, @"province": self.provinceStr, @"city": self.cityStr,@"area": self.townStr,@"isDefault":@"",@"postCode":@"",@"receiverName":self.nametext.text,@"receiverPhone":self.phonetext.text,@"addressId":self.listdata.addressId };
    
    [MineRequestTool PutPanetMineaddressnParameter:parameter success:^(PanetKnInetlCommon * _Nonnull respone) {
...
{
    _QalertView = [[BBTQAlertView alloc] initWithBBTTitle:@"温馨提示" andWithMassage:@"您确定要删除收货人吗?" andWithTag:1 andWithButtonTitle:@"取消",@"确定",nil];
    [_QalertView showInView:self.view];
    
    __block PanetMineEditAddressViewController *self_c = self;
    //点击按钮回调方法
...
//            NSDictionary *parameter = @{@"contactsId":self_c.FamilyContact.contactsId};
            ...
            [MineRequestTool DeletePanetMineaddressnAddressId:self_c.listdata.addressId success:^(PanetKnInetlCommon * _Nonnull respone) {
 ...
 replacementText:(NSString *)text
{
    ...
    if (selectedRange && pos) {
        ...
        if (offsetRange.location < PanetMineEditAddress_MAX_LIMIT_NUMS) {
            return YES;
        }
        else
        {
    ...
    NSInteger caninputlen = PanetMineEditAddress_MAX_LIMIT_NUMS - comcatstr.length;
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (existTextNum > PanetMineEditAddress_MAX_LIMIT_NUMS)
    {
        //截取到最大位置的字符
        NSString *s = [nsTextContent substringToIndex:PanetMineEditAddress_MAX_LIMIT_NUMS];
