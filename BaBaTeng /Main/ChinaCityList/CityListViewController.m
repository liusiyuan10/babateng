...
- (void)viewDidLoad
{
    ...
    /**
     ...
     //搜索框
     _searchText = [[UITextField alloc]initWithFrame:CGRectMake(30,20, self.view.frame.size.width-60,45)];
     _searchText.backgroundColor = [UIColor whiteColor];
     _searchText.font = [UIFont systemFontOfSize:15];
     _searchText.placeholder  = @"请输入城市名称或首字母查询";
     [_searchText setValue:[UIColor grayColor] forKeyPath:@"_placeholderLabel.textColor"];
     [_searchText setValue:[UIFont systemFontOfSize:15] forKeyPath:@"_placeholderLabel.font"];
     
     UIView *lv = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 20, 50)];//宽度根据需求进行设置，高度必须大于 textField 的高度
     ...
     _searchText.layer.borderColor   = [UIColor colorWithRed:220/255.0 green:220/255.0 blue:220/255.0 alpha:1].CGColor;
     _searchText.layer.borderWidth   = 1;
     _searchText.returnKeyType = UIReturnKeySearch;
     _searchText.textColor    =[UIColor colorWithRed:220/255.0 green:220/255.0 blue:220/255.0 alpha:1];
...
- (void)ininHeaderView
{
    ...
    _locatingCityGroupView.columns = 4;
    ...
    //
    //
    //    long rowHistorical = _arrayHistoricalCity.count/4;
    //    if (_arrayHistoricalCity.count%4 > 0) {
    //        rowHistorical += 1;
    //    }
    ...
    //    _historicalCityGroupView.columns = 4;
    ...
    long row = _arrayHotCity.count/4;
    if (_arrayHotCity.count%4 > 0) {
        row += 1;
    }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    bgView.backgroundColor = [UIColor clearColor];//BGCOLOR;
    ...
    UIImageView *line = [[UIImageView alloc]initWithFrame:CGRectMake(0, 29, bgView.frame.size.width, 1)];
...
- (void)showTipsWithTitle:(NSString*)title
{
    ...
    if (!_tipsView) {
        ...
        _tipsView.layer.borderColor   = [UIColor grayColor].CGColor;
