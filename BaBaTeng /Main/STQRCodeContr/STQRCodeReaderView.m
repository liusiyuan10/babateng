...
- (void)layoutSubviews
{
    ...
    self.labelAlert.center = CGPointMake(self.imageScanZone.center.x, self.imageScanZone.frame.origin.y - 30);
}
...
{
    //手动输入设备条码
    if (self.delegate && [self.delegate respondsToSelector:@selector(manuallyEnter)]) {
        [self.delegate manuallyEnter];
    }
...
- (void)startAnimation
{
    CGFloat viewW = 220*ST_QRCODE_WidthRate;
    CGFloat viewH = 3;
    ...
    [UIView animateWithDuration:1.5 delay:0 options:UIViewAnimationOptionRepeat animations:^{
        viewY = (ST_QRCODE_ScreenHeight- viewW)/2 + 220*ST_QRCODE_WidthRate - 5;
...
- (UIImageView *)imageScanZone{
    if (!_imageScanZone) {
        ...
        _imageScanZone = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"icon_Scanframe"]];
    }
...
- (CGRect)rectScanZone
{
    return CGRectMake(50*ST_QRCODE_WidthRate, (ST_QRCODE_ScreenHeight-220*ST_QRCODE_WidthRate)/2, 220*ST_QRCODE_WidthRate, 220*ST_QRCODE_WidthRate);
}
...
- (UIImageView *)imageMove
{
    if (!_imageMove) {
        ...
        _imageMove.image = [UIImage imageNamed:@"icon_strip"];
    }
    return _imageMove;
}
...
- (UIButton *)buttonTurn
{
    if (!_buttonTurn) {
        ...
        //[_buttonTurn setTitle:@" 手动输入设备码 " forState:UIControlStateNormal];
        //        _buttonTurn.layer.borderWidth =1;
        //        _buttonTurn.layer.cornerRadius =5;
        //        _buttonTurn.layer.borderColor = [UIColor whiteColor].CGColor;
        //        [_buttonTurn setBackgroundImage:[NSBundle st_qrcodeControllerImageWithName:@"icon_inputcode"] forState:UIControlStateNormal];
        //        [_buttonTurn setBackgroundImage:[NSBundle st_qrcodeControllerImageWithName:@"icon_inputcode"] forState:UIControlStateSelected];
        
        [_buttonTurn setImage:[UIImage imageNamed:@"icon_inputcode"] forState:UIControlStateNormal];
        [_buttonTurn setImage:[UIImage imageNamed:@"icon_inputcode"] forState:UIControlStateHighlighted];
...
- (UILabel *)labelAlert
{
    if (!_labelAlert) {
        ...
        [_labelAlert setText:@"将二维码放置框内，即开始扫描"];
