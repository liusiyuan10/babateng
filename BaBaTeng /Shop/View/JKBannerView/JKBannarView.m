...
@property (strong, nonatomic) UILabel *mynumLabel;
...
//        [self addSubview:_mypageControl];
        ...
        self.mynumLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.viewSize.width-30 - 17, self.viewSize.height-21-16 , 30 , 16)];
        
        self.mynumLabel.font = [UIFont systemFontOfSize:10.0];
        self.mynumLabel.backgroundColor = [UIColor colorWithRed:144/255.0 green:144/255.0 blue:144/255.0 alpha:1.0];
        self.mynumLabel.textColor = [UIColor whiteColor];
        self.mynumLabel.textAlignment = NSTextAlignmentCenter;
       
        self.mynumLabel.text = [NSString stringWithFormat:@"1/%lu",(unsigned long)_items.count];
        self.mynumLabel.layer.masksToBounds = YES; //没这句话它圆不起来
        self.mynumLabel.layer.cornerRadius = 8; //设置图片圆角的尺度
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:_items[indexPath.row]] placeholderImage:[UIImage imageNamed:@"classdetail_empty"]];
    return cell;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    int page = (int)(scrollView.contentOffset.x / scrollView.frame.size.width + 0.5) % _items.count + 1;
    self.mynumLabel.text = [NSString stringWithFormat:@"%d/%lu",page,(unsigned long)_items.count];
