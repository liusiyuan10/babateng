...
- (void)initUI {
    ...
    [self.timeNowLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(weakself).offset(13);
        make.centerY.equalTo(weakself);
        make.height.mas_equalTo(15);
        make.width.mas_equalTo(37);
    }];
    
    self.timeSlider = [[MusicSlider alloc] initWithFrame:CGRectMake(87-30, 0, weakself.frame.size.width - 142+30, self.frame.size.height)];
    ...
    self.timeSlider.trackBackgoundColor =[UIColor colorWithRed:246/255.0 green:234/255.0 blue:221/255.0 alpha:1.0];
    self.timeSlider.playProgressBackgoundColor = NavBackgroundColor;
    self.timeSlider.progressBackgoundColor =TrackColor;
...
- (void)initPlayerItem {
    if (self.musicModel && ![self.musicModel.musicURL isEqualToString:@""]) {
        ...
        NSString *encodedString = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)self.musicModel.musicURL, (CFStringRef)@"!NULL,'()*+,-./:;=?@_~%#[]", NULL, kCFStringEncodingUTF8));
        NSLog(@"self.musicModel.musicURL===%@",self.musicModel.musicURL);
        
        self.playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:encodedString]];
        [self.player replaceCurrentItemWithPlayerItem:self.playerItem];
        NSLog(@"self.playerItem111 ===%@",self.playerItem);
    }
}
...
- (void)addPlayerListener {
    ...
    if (self.playerItem) {
        ...
        self.timeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 30) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
            ...
            if ([weakSelf.musicPlayerDelegate respondsToSelector:@selector(playerViewTimeNow:)]) {
                [weakSelf.musicPlayerDelegate playerViewTimeNow:currentPlayTime];
            }
        }];
...
-(void)playerFinished:(NSNotification *)notification{
    ...
    NSString *stauestr = [[TMCache sharedCache] objectForKey:@"ISPLAYBLOCK"];
    
    if ([stauestr isEqualToString:@"block"]) {
        return;
    }
    ...
    if ([self.musicPlayerDelegate respondsToSelector:@selector(playerViewFinished)]) {
        [self.musicPlayerDelegate playerViewFinished];
    }
...
-(void)playerFailed{
    ...
    if ([self.musicPlayerDelegate respondsToSelector:@selector(playerViewFailed)]) {
        [self.musicPlayerDelegate playerViewFailed];
    }
...
- (void)appEnteredBackground {
    ...
    }else if (self.player && self.playerStatus == VedioStatusFinished) {
...
- (void)sliderScrubbing {
    if (self.totalTime != 0) {
        ...
        if ([self.musicPlayerDelegate respondsToSelector:@selector(playerViewTimeNow:)]) {
            [self.musicPlayerDelegate playerViewTimeNow:self.timeSlider.value];
        }
    }
}
