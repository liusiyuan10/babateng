...
@interface SearchTapSliderScrollView()<UIScrollViewDelegate>

@end


@implementation SearchTapSliderScrollView
{
...
#pragma mark 创建视图
-(void)createView:(NSArray *)titleArr andViewArr:(NSArray *)viewArr SearchStr:(NSString *)searchstr andRootVc:(UIViewController *)rootVC
{
    //当前的选中下标
    ...
    for (int i = 0; i<titleArr.count; i++) {
        ...
        CommonViewController *contr=viewArr[i];
        contr.searchstr = searchstr;
        //添加子view
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(SearchsliderViewAndReloadData:)] ) {

        [self.delegate SearchsliderViewAndReloadData:currentAdPage];
...
-(void)changeAnimation:(UIButton *)sender
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(SearchsliderViewAndReloadData:)] ) {
        ...
        [self.delegate SearchsliderViewAndReloadData:index];
