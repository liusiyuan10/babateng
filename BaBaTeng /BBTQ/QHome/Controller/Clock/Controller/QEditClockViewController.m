...
@interface QEditClockViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation QEditClockViewController
...
//    self.unselect_week_img = @[@"clock_Mo_n",@"clock_Tu_n",@"clock_We_n",@"clock_Th_n",@"clock_Fr_n",@"clock_Sa_n",@"clock_Su_n"];
    ...
    self.clockBellStr = self.clockdata.clockSound.soundName;
    self.clockClueStr = self.clockdata.songname;
    self.clockBellId = self.clockdata.soundId;
    self.enableStr = self.clockdata.enable;
    self.timeStr = self.clockdata.time;
    
    self.weekBtnArr = [[NSMutableArray alloc] init];
    
    self.weekArr = [[NSMutableArray alloc] init];
    
    NSArray *strarray = [self.clockdata.repeat componentsSeparatedByString:@","];
    for (int i = 0; i < strarray.count; i++) {
        NSString *str = [strarray objectAtIndex:i];
        [self.weekArr addObject:str];
        
    }
//    self.weekArr = (NSMutableArray *)[self.clockdata.repeat componentsSeparatedByString:@","];
    
    
    self.selectedDate = [self ClockDateStr:self.clockdata.time];
...
//    [self.clockDatePicker setCalendar:[NSCalendar currentCalendar]];
...
- (void)storageBtnClicked
{
    ...
    //    deviceId    设备编号    string    @mock=0100060000000001
    //    enable    启用关闭闹钟    boolean    @mock=true
    //    repeat    重复时间    string    @mock=1,2,3,4 1：表示周日，7：表示周六
    //    songname    闹钟提示语    string    @mock=起床了
    //    soundId    闹钟铃声编号    number    @mock=1
    //    time    闹钟时间    string    @mock=18:00
...
//    }
    ...
    NSDictionary *parameter = @{@"enable" : self.enableStr, @"clockId": self.clockdata.clockId,@"deviceId":self.clockdata.deviceId ,@"repeat": self.repeatStr,@"songname": self.clockClueStr,@"soundId":self.clockBellId,@"time":self.timeStr};
...
- (NSDate *)ClockDateStr:(NSString *)str
{
