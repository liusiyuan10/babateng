...
@interface AlbumTapSliderScrollView()<UIScrollViewDelegate>

@end


@implementation AlbumTapSliderScrollView
{
...
#pragma mark 创建视图
-(void)createView:(NSArray *)titleArr andViewArr:(NSArray *)viewArr DeviceTypeId:(NSString *)deviceTypeId PanelId:(NSString *)paneId andRootVc:(UIViewController *)rootVC
{
    //当前的选中下标
    ...
    for (int i = 0; i<titleArr.count; i++) {
        ...
        funcBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
        ...
        funcBtn.frame = CGRectMake( i*_width, 3, _width, 40);
        
        [self addSubview:funcBtn];
        
        NewQAlbumListViewController *contr=viewArr[i];
        contr.panelId = paneId;
        contr.deviceTypeId = deviceTypeId;
        contr.orderBy = [NSString stringWithFormat:@"%d", i];
    ...
    self.LbuttonViewArr = _buttonViewArr;
    ...
    _lineView.frame = CGRectMake(0, 40, _width *4/5.0, 2);
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(AlbumsliderViewAndReloadData:)] ) {

        [self.delegate AlbumsliderViewAndReloadData:currentAdPage];
...
-(void)changeAnimation:(UIButton *)sender
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(AlbumsliderViewAndReloadData:)] ) {
        ...
        [self.delegate AlbumsliderViewAndReloadData:index];
...
-(void)scrollToIndex:(NSInteger)index
{
    ...
    else
    {
        ...
        selectBtn.titleLabel.font = [UIFont boldSystemFontOfSize:16];
        lastSelectBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    }
