...
@interface QEditContactViewController ()<UITableViewDelegate,UITableViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate,VPImageCropperDelegate,CustomSheetViewDelegate>
{
...
@implementation QEditContactViewController
...
//        _IconView.image = [UIImage imageNamed:@"Oval Copy"];
        
        NSString *encodedString = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)self.Icon, (CFStringRef)@"!NULL,'()*+,-./:;=?@_~%#[]", NULL, kCFStringEncodingUTF8));
        
        [_IconView setImageWithURL:[NSURL URLWithString:encodedString] placeholderImage:[UIImage imageNamed:@"BBZL_icon_touxian"]];
...
-(UITextField *)numTextField
{
    if (_numTextField == nil) {
        ...
        _numTextField.text = self.FamilyContact.phoneNumber;
...
-(UILabel *)nameLabel
{
    if (_nameLabel == nil) {
        ...
        _nameLabel.text = self.FamilyContact.nickName;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(QRoleRefresh:) name:@"QRoleRefresh" object:nil];
    
    self.nicknameId = self.FamilyContact.nicknameId;
    self.Icon = self.FamilyContact.icon;
    self.RoleName = self.FamilyContact.nickName;
    self.IsCommn = self.FamilyContact.isCommon;
...
//    self.tableView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    ...
    self.saveBtn = [[UIButton alloc] initWithFrame:CGRectMake(35, CGRectGetMaxY(self.tableView.frame) + 20 /667.0 *KDeviceHeight, kDeviceWidth - 70, 74)];
...
//    self.saveBtn.backgroundColor = [UIColor redColor];
    ...
    self.DeleteLabel = [[UILabel alloc] initWithFrame:CGRectMake( (kDeviceWidth - 100)/2.0, CGRectGetMaxY(self.saveBtn.frame), 100, 20)];
    
    self.DeleteLabel.textColor = [UIColor colorWithRed:102/255.0 green:102/255.0 blue:102/255.0 alpha:1.0];
    self.DeleteLabel.font = [UIFont systemFontOfSize:14.0];
    self.DeleteLabel.text = @"删除联系人";
    self.DeleteLabel.textAlignment = NSTextAlignmentCenter;
    self.DeleteLabel.userInteractionEnabled = YES;
    
    UITapGestureRecognizer *deleteTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(DeleteLabelClicked)];
...
//    NSDictionary *parameter = @{@"phoneNumber" : self.numTextField.text, @"deviceId": [[TMCache sharedCache] objectForKey:@"deviceId"], @"icon": self.Icon,@"isCommon": self.IsCommn,@"nickName":self.nameLabel.text,@"nicknameId":self.nicknameId ,@"contactsId":self.FamilyContact.contactsId};
    
    NSDictionary *parameter = @{@"phoneNumber" : self.numTextField.text, @"deviceId": [[TMCache sharedCache] objectForKey:@"deviceId"], @"icon": self.Icon,@"isCommon": self.IsCommn,@"nicknameId":self.nicknameId,@"contactsId":self.FamilyContact.contactsId};

    
    NSLog(@"parameter123456========%@",parameter);
    
    [self startLoading];
    [QFamilyCallRequestTool EditfamilyContactsParameter:parameter success:^(QFamilyEditContact *response) {
        [self stopLoading];
...
{
    _QalertView = [[BBTQAlertView alloc] initWithBBTTitle:@"温馨提示" andWithMassage:@"您确定要删除联系人吗?" andWithTag:1 andWithButtonTitle:@"取消",@"确定",nil];
    [_QalertView showInView:self.view];
    
    __block QEditContactViewController *self_c = self;
    //点击按钮回调方法
    _QalertView.resultIndex = ^(NSInteger titleBtnTag,NSInteger alertViewTag){
        if (titleBtnTag == 1) {
            ...
            NSLog(@"删除联系人");
            NSDictionary *parameter = @{@"contactsId":self_c.FamilyContact.contactsId};
        
            [self_c startLoading];
        
            [QFamilyCallRequestTool DeletefamilyContactsParameter:parameter success:^(QFamilyCommon *response) {
        
        
                [self_c stopLoading];
                if ([response.statusCode isEqualToString:@"0"]) {
        
                    [self_c showToastWithString:@"删除成功"];
...
//    NSLog(@"删除联系人");
//    NSDictionary *parameter = @{@"contactsId":self.FamilyContact.contactsId};
//
//    [self startLoading];
//
//    [QFamilyCallRequestTool DeletefamilyContactsParameter:parameter success:^(QFamilyCommon *response) {
...
- (void)QRoleRefresh:(NSNotification *)noti
{
...
//            cell.textLabel.text = self.RoleName;
        ...
        case 3:
            ...
            if ([self.IsCommn isEqualToString:@"1"]) {
                self.switchview.on = YES;
            }else
            {
