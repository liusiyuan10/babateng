...
@interface QSongListViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation QSongListViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(QSongList:) name:@"QSongList" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(QSongListPlay:) name:@"QSongListPlay" object:nil];
...
//    NSLog(@"只dsdfdsg");
//    
//    [self performSelector:@selector(getReSong) withObject:nil afterDelay:1.0];
...
- (void)QSongListPlay:(NSNotification *)noti
{
...
//    }
    ...
    for (int i = 0; i < self.SongArticles.count; i++) {
        ...
        QAlbumDataTrackList *listrespone = [self.SongArticles objectAtIndex:i];
...
- (void)QSongList:(NSNotification *)noti
{
...
- (void)AddDemandListSongListId:(NSString *)songListId TrackId:(NSString *)trackid
{
    ...
    NSDictionary *parameter = @{@"userId" : [[TMCache sharedCache] objectForKey:@"userId"] , @"token" : [[TMCache sharedCache] objectForKey:@"token"],@"listId" :songListId  ,@"trackId" : trackid, @"deviceId" : [[TMCache sharedCache] objectForKey:@"deviceId"],@"type" :@"0"};
...
//    resultRespone2= [self.SongArticles objectAtIndex:btn.tag];
//    
...
//        for (int i=0; i<self.SongArticles.count; i++) {
//            
//            resultRespone1= [self.SongArticles objectAtIndex:i];
...
//
    ...
    if (btn.selected) {
        if (resultRespone.IsDeviceplay) {
           ...
           // NSLog(@"sb");
            [self AddDemandListSongListId:self.QResponse.deviceSongListId TrackId:resultRespone.trackId];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.strTracid.length !=0) {
        ...
        for (int i = 0; i < self.SongArticles.count; i++) {
            ...
            QAlbumDataTrackList *listrespone1 = [self.SongArticles objectAtIndex:i];
