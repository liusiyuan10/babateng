...
@interface QAlbumViewController ()<QSegmentToolCellDelegate,UITableViewDelegate,UITableViewDataSource,QAlbumListCellDelegate,UIGestureRecognizerDelegate,QAlbumDetailViewDelegate,QPlayListViewDelegate,MusicPlayerViewDelegate>{
...
@implementation QAlbumViewController
...
//    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(50, 100, 30, 30)];
...
//    [self.view addSubview:btn];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(DemandAlbum:) name:@"DemandAlbum" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(DemandAlbumPlay:) name:@"DemandAlbumPlay" object:nil];
    ...
    for(int i = 0;i<[self.headViewArray count];i++)
    {
        ...
        if(head.section == trackIdIndex)
        {
            if ([strtest1 isEqualToString:@"playing"])
            {
                ...
                NSLog(@"trakidindex11111111=======%d",trackIdIndex);
            ...
            else
            {
                ...
                NSLog(@"trakidindex2222222222=======%d",trackIdIndex);
                IsRePalying = NO;
                head.nameLabel.textColor = NavBackgroundColor;
...
- (void)DemandAlbum:(NSNotification *)noti
{
    ...
    NSLog(@"strtest112222222222233333333====%@",strtest1);
...
- (void)GetRTrackLists
{
    ...
    NSString *Pagesize = [NSString stringWithFormat:@"%ld",(long)self.totalNum];
...
//    CGFloat EvaltextX = 80;
//    CGFloat EvaltextY = CGRectGetMaxY(_timeFrame) + 8;
//    CGFloat EvaltextW = kDeviceWidth - 104;
//
//
//    CGSize EvaltextSize = [evaluationRespone.content sizeWithMaxSize:CGSizeMake(EvaltextW, MAXFLOAT) fontSize:14.0];
//
//
//    _EvaltextFrame  = (CGRect){{EvaltextX,EvaltextY},EvaltextSize};
