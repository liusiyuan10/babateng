...
@class BPlayActionSheet;
///Block回调
typedef void (^SelectIndexBlock)(NSInteger index, QAlbumDataTrackList *title);

typedef NS_ENUM(NSUInteger, BPlaySheetStyle) {
    ///默认样式
    BPlaySheetStyleDefault = 0,
    ///像微信样式
    BPlaySheetStyleWeiChat,
    ///TableView样式(无取消按钮)
    BPlaySheetStyleTable,
};
...
@protocol BPlayActionSheetDelegate <NSObject>
...
- (void)sheetViewDidSelectIndex:(NSInteger)index
                         ...
                         sender:(BPlayActionSheet *)actionSheet;
...
@interface BPlayActionSheet : UIView
...
@property (nonatomic, assign) id<BPlayActionSheetDelegate> zActionDelegate;
