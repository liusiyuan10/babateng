...
- (DropMenuView *)initWithOrigin:(CGPoint)origin {
    self = [self initWithFrame:CGRectMake(0, origin.y, SCREEN_WIDTH , SCREEN_HEIGHT - origin.y)];
    if (self) {
        _origin = origin;
...
//    _tableView.frame = CGRectMake(_origin.x, 0, self.frame.size.width, [self.menuArray count] * tableViewCellHeight);
    ...
    CGFloat rowHO = self.frame.size.height - 155;
    CGFloat rowHN = [self.menuArray count] * tableViewCellHeight;
    CGFloat rowH = 0;
    if (rowHO > rowHN) {
        rowH = rowHN;
    }else
    ...
    //    _tableView.frame = CGRectMake(_origin.x, 0, self.frame.size.width, [self.menuArray count] * tableViewCellHeight);
    //
    _tableView.frame = CGRectMake(_origin.x, 0, self.frame.size.width, rowH);
    
//
//     _tableView.frame = CGRectMake(_origin.x, 0, self.frame.size.width, self.frame.size.height - 155);
...
//    self.filterId =   [[TMCache sharedCache] objectForKey:@"HomeFilterId"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.filterId isEqualToString:model.filterId]) {
