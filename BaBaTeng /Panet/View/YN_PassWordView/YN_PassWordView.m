...
-(void)initShow5{
    ...
    for (int i =0; i<self.num; i++) {
        UIView *lineV = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.textF.frame) + (i)*(width+f), K_Field_Height-1,width,2)];
        lineV.backgroundColor = [UIColor colorWithRed:136/255.0 green:143/255.0 blue:139/255.0 alpha:1.0];//原始线的颜色
        [self addSubview:lineV];
    ...
    for (int i=0; i<self.num; i++) {
        ...
        codeL.backgroundColor = [UIColor whiteColor];
        codeL.textColor = [UIColor colorWithRed:89/255.0 green:89/255.0 blue:89/255.0 alpha:1.0];
        codeL.font = [UIFont systemFontOfSize:27];
        codeL.hidden = YES;
        codeL.textAlignment = NSTextAlignmentCenter;
        [self addSubview:codeL];
        [self.codeLArr addObject:codeL];
